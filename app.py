...
def ensure_sheet_header() -> None:
    ...
    expected = ["timestamp", "order_id", "customer_name", "email", "phone", "delivery_method", "payment_method", "street_address", "postal_code", "city", "notes", "items", "total_eur", "language"]
    if worksheet.row_values(1) != expected:
        worksheet.update("A1:N1", [expected])

def save_order(customer_name: str, email: str, phone: str, delivery_method: str, payment_method: str, street_address: str, postal_code: str, city: str, notes: str, products: pd.DataFrame) -> tuple[str, str, float, str]:
    ...
    worksheet.append_row([timestamp, order_id, customer_name, email, phone, delivery_method, payment_method, street_address, postal_code, city, notes, items, total, st.session_state.lang])
...
{t('receipt_time')}: {order_data['timestamp']}
{t('receipt_customer')}: {order_data['customer_name']}
{t('email').replace(' *','')}: {order_data['email']}
{t('phone')}: {order_data['phone']}
{t('delivery_method')}: {order_data['delivery_method']}
{t('payment_pref_label')}: {order_data['payment_method']}
{t('receipt_address')}: {address_block}

{t('receipt_items')}:
{item_lines}

{t('total')}: {euro_fi(order_data['total'])}
...
def validate_order_form(customer_name: str, email: str, phone: str, delivery_method: str, street_address: str, postal_code: str, city: str, honeypot: str) -> str | None:
    if honeypot.strip():
...
def checkout_form(products: pd.DataFrame) -> None:
    ...
    with st.form("checkout_form"):
        ...
        if submitted:
            validation_error = validate_order_form(customer_name, email, phone, delivery_method, street_address, postal_code, city, honeypot)
            if validation_error:
                st.error(validation_error)
                return
            try:
                with st.spinner(t("save_order")):
                    order_id, timestamp, total, items = save_order(customer_name.strip(), email.strip(), phone.strip(), delivery_method, payment_method, street_address.strip(), postal_code.strip(), city.strip(), notes.strip(), products)
                st.session_state.last_submit_ts = time.time()
                st.session_state.last_email_error = None
                try:
                    with st.spinner(t("notify_shop")):
                        send_owner_notification(customer_name.strip(), email.strip(), phone.strip(), delivery_method, payment_method, street_address.strip(), postal_code.strip(), city.strip(), notes.strip(), order_id, timestamp, total, products)
                except Exception as e:
                    st.session_state.last_email_error = str(e)
                st.session_state.last_order = {"order_id": order_id, "timestamp": timestamp, "customer_name": customer_name.strip(), "email": email.strip(), "phone": phone.strip(), "delivery_method": delivery_method, "payment_method": payment_method, "street_address": street_address.strip(), "postal_code": postal_code.strip(), "city": city.strip(), "items": items, "total": total}
                clear_cart()
                st.balloons()
                st.rerun()
            except Exception:
